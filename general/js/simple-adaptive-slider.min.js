...
let sliderIndicator = document.querySelector(".slider-indicator-slider");
let sliderIndicatorBody = document.querySelector(".slider-indicator-body");

let slashes = document.getElementsByClassName("slash");
let leftSliderButton = document.querySelector(".left-slider-button");
let rightSliderButton = document.querySelector(".right-slider-button");
let slider;

function animationLeftSliderButton() {
  if (window.getComputedStyle(leftSliderButton, null).getPropertyValue("background-color") == "rgba(255, 255, 255, 0)") {
    leftSliderButton.style.backgroundColor = "rgba(255, 255, 255, 0.5)";
    for(let slash of leftSliderButton.getElementsByClassName("slash")){
      // slash.style.opacity = 0;
    }
  }
  setTimeout(() => {
    if (window.getComputedStyle(leftSliderButton, null).getPropertyValue("background-color") == "rgba(255, 255, 255, 0.5)") {
      leftSliderButton.style.backgroundColor = "rgba(255,255,255,0)";
      for(let slash of leftSliderButton.getElementsByClassName("slash")){
...
function animationRightSliderButton() {
  if (window.getComputedStyle(rightSliderButton, null).getPropertyValue("background-color") == "rgba(255, 255, 255, 0)") {
    rightSliderButton.style.backgroundColor = "rgba(255, 255, 255, 0.5)";
    for(let slash of rightSliderButton.getElementsByClassName("slash")){
      // slash.style.opacity = 0;
    }
  }
  setTimeout(() => {
    if (window.getComputedStyle(rightSliderButton, null).getPropertyValue("background-color") == "rgba(255, 255, 255, 0.5)") {
      rightSliderButton.style.backgroundColor = "rgba(255,255,255,0)";
      for(let slash of rightSliderButton.getElementsByClassName("slash")){
        // slash.style.opacity = 1;
      }
    }
...
function moveWithLeftButton() {
  leftSliderButton.onclick = () => {
    slider.prev();
    animationLeftSliderButton();
  };
}

function moveWithRightButton() {
  rightSliderButton.onclick = () => {
    slider.next();
    animationRightSliderButton();
  };
}

function initialSliderButtons() {
  leftSliderButton.style.backgroundColor = "rgba(255, 255, 255, 0)";
  rightSliderButton.style.backgroundColor = "rgba(255, 255, 255, 0)";
  leftSliderButton.style.transitionDuration = "0.15s";
  rightSliderButton.style.transitionDuration = "0.1s";
  for (let slash of slashes) {
    slash.style.transitionDuration = "0.2s";
    slash.style.margin = "-" + (window.getComputedStyle(document.querySelector(".itcss"), null).getPropertyValue("width").slice(0, -2) * 0.0145) + "px";
    console.log(window.getComputedStyle(document.querySelector(".itcss"), null).getPropertyValue("width").slice(0, -2));
  }
}
...
class ItcSimpleSlider {
  ...
  _changeActiveItems() {
    this._states.forEach((item, index) => {
      ...
      this.moveSliderIndicator(item, index);
      this.changeIndicatorColor(item, index);
    });
    this._el.dispatchEvent(new CustomEvent('change.itc.slider', { bubbles: true }));
  }


  moveSliderIndicator(item, index) {
    let sliderIndicator = document.querySelector(".slider-indicator-slider");
    if (item == 1) {
      sliderIndicator.style.marginLeft = index * window.getComputedStyle(sliderIndicator).width.slice(0, -2) + "px";
    }

  }

  changeIndicatorColor(item, index) {
    let sliderIndicatorNumbers = document.querySelector(".slider-indicator-num");
    if (item == 1) {
      for (let num of sliderIndicatorNumbers.children) {
        num.style.color = "#b0b0b0";
      }
      sliderIndicatorNumbers.children[index].style.color = "#000";
    }
  }
